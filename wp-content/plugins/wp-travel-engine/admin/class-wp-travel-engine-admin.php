...
class Wp_Travel_Engine_Admin {
	...
	public function enqueue_scripts() {
    	...
    	{
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/wp-travel-engine-admin.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-tabs', 'jquery-ui-sortable' ), $this->version, true );
			$extra_array = array(
					'lang' => array(
			...
			wp_enqueue_script( $this->plugin_name.'media-logo-upload', plugin_dir_url( __FILE__ ) . 'js/media-upload.js', array( 'jquery' ), $this->version, true );
		}
		...
		{
			wp_enqueue_script( $this->plugin_name.'gallery-metabox', plugin_dir_url( __FILE__ ) . 'js/gallery-metabox.js', array( 'jquery' ), $this->version, true );
		}
		wp_enqueue_media();
    ...
    */
	...
	{ 
		...
		$nonce = $_POST['nonce'];
            ...
            $response .= '<span class="handle"><span></span></span>';
    ...
    */
	function wpte_get_destination_template( $template ) {
        ...
        if( $page_template == 'templates/template-destination.php' ){
            ...
            if(!$template_path)
            {
                $template_path = WP_TRAVEL_ENGINE_BASE_PATH . '/includes/templates/template-destination.php';
            }
            return $template_path;
        }
        if( $page_template == 'templates/template-activities.php' ){
            $template_path = locate_template( array( 'template-activities.php' ) );
            if(!$template_path)
            {
                $template_path = WP_TRAVEL_ENGINE_BASE_PATH . '/includes/templates/template-activities.php';
            }
            return $template_path;
            
        }
        if( $page_template == 'templates/template-trip_types.php' ){
            $template_path = locate_template( array( 'template-trip_types.php' ) );
            if(!$template_path)
            {
                $template_path = WP_TRAVEL_ENGINE_BASE_PATH . '/includes/templates/template-trip_types.php';
            }
            return $template_path;
        }
        if( $page_template == 'templates/template-trip-listing.php' ){
            $template_path = locate_template( array( 'template-trip-listing.php' ) );
            if(!$template_path)
            {
                $template_path = WP_TRAVEL_ENGINE_BASE_PATH . '/includes/templates/template-trip-listing.php';
            }
            return $template_path;
        }
        return $template;
	...
	function wpte_add_itinerary_template() { 
		...
		<div id="itinerary-template">
			<li id="itinerary-tabs{{index}}" data-id="{{index}}" class="itinerary-row">
		        ...
		        <a class="accordion-tabs-toggle" href="javascript:void(0);"><span class="day-count"><?php _e('Day','wp-travel-engine');echo '-{{index}}';?></span></a>
	...
	function wpte_add_faq_template() { 
		...
		<div id="faq-template">
			<li id="faq-tabs{{index}}" data-id="{{index}}" class="faq-row">
		        ...
		        <a class="accordion-tabs-toggle" href="javascript:void(0);"><span class="day-count"><?php _e('FAQ','wp-travel-engine');echo '-{{index}}';?></span></a>
